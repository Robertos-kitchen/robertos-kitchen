...
// date, 'YYYY-MM-DD', so the kitchen can be counted at ANY cadence — monthly
// today, but weekly, twice-monthly or daily would need no further change (the
// bar already counts twice a month). Each date is its own record: uploading a
// list for a NEW date starts a new count and cannot touch an earlier one.
//
...
// newest first, by the real date — NOT by the key as text. A legacy month label
// ('2026-07' = 31 July) sorts before a mid-month count of the same month
// ('2026-07-15') as text, which would open the wrong count.
...
var stPrevMonth = null;      // key of the count being compared against (null if none)
var stPrevLabel = '';        // e.g. '30 June 2026'
var stPrevOptions = [];      // every earlier count available to compare against, newest first
var stPrevChoice  = null;    // { key } when Aung picked one himself (else the most recent)
...
// Every earlier count, newest first — "earlier" decided on the real date, since
// a .lt() on the text would treat 15 July as later than the July month-end.
async function stLoadPrevOptions(){
  stPrevOptions = [];
  if(!stMonth) return;
  var now = stKeyDate(stMonth); if(!now) return;
  var res = await sb.from('stock_take_sheets').select('month')
    .eq('venue_id',STOCK_VENUE).eq('dept',STOCK_DEPT);
  stSortKeysDesc(res.data).forEach(function(s){
    var d = stKeyDate(s.month);
    if(d && d.getTime() < now.getTime()) stPrevOptions.push({ key:s.month });
  });
  if(stPrevChoice && !stPrevOptions.some(function(o){ return o.key===stPrevChoice.key; })) stPrevChoice = null;
}
async function stLoadPrevMonth(){
  ...
  // Default = the count immediately before this one, whatever the gap. Aung can
  // pick any earlier count instead.
  var chosen = null;
  if(stPrevChoice) chosen = stPrevOptions.find(function(o){ return o.key===stPrevChoice.key; });
...
function stRender(){
  ...
  view.innerHTML =
    ...
    (stUser? '<div class="st-actions">'+
        ...
        (stCanCompare() ? '<button class="report-btn" onclick="stShowCompare()">📊 Compare with previous count</button>' : '')+
...
async function stLockMonth(){
  ...
  stRender();
  if(typeof kToast==='function') kToast('🔒 '+stPeriodLabel(stMonth)+' is now locked.');
}
async function stUnlockMonth(){
  ...
  stRender();
  if(typeof kToast==='function') kToast('🔓 '+stPeriodLabel(stMonth)+' unlocked.');
}
...
async function stClearAllCounts(){
  ...
  stCounts={};
  stRender();
  if(typeof kToast==='function') kToast('✓ All counts cleared for '+stPeriodLabel(stMonth)+'.');
}
...
// Say the real gap in words. The kitchen counts monthly today, but the moment it
// counts fortnightly "last month" would be wrong — and a 15-day movement read as
...
function stShowCompare(){
  ...
  if(!stPrevMonth){
    box.innerHTML='<div class="st-cmp-box" onclick="event.stopPropagation()">'+
      '<div class="st-cmp-head"><div><div class="st-cmp-title">Compare with previous count</div></div>'+
      '<button class="report-btn" onclick="document.getElementById(\'st-cmp-modal\').remove()">Close</button></div>'+
      '<div class="report-no-data">There is no earlier stock take in the system yet, so there is nothing to compare '+stEsc(stPeriodLabel(stMonth))+' against. The comparison will work from the next count onwards.</div></div>';
  } else {
    box.innerHTML='<div class="st-cmp-box" onclick="event.stopPropagation()">'+
      '<div class="st-cmp-head">'+
        '<div><div class="st-cmp-title">'+stEsc(stPrevLabel)+' → '+stEsc(stPeriodLabel(stMonth))+'</div>'+
        '<div class="st-cmp-sub">'+stEsc(stCmpGapText())+' Count vs count. This is <b>movement</b>, not usage or loss — this app holds counts only, not purchases or sales.</div></div>'+
...
function stRenderCompare(){
  ...
  var head =
    ...
    '</div>'+
    (uncounted>0 ? '<div class="st-cmp-warn">⚠ '+uncounted+' of '+stItems.length+' items are still not counted, so the difference above will keep moving. Read the list below as work-in-progress until the count is finished.</div>' : '')+
      ...
      (stPrevOptions.length>1 ? '<label>Compare against '+
        '<select class="st-select" style="height:32px" onchange="stCmpSetPrev(this.value)">'+
          stPrevOptions.map(function(o,i){
            return '<option value="'+i+'"'+(o.key===stPrevMonth?' selected':'')+'>'+stEsc(stPeriodShort(o.key))+(i===0?' — previous count':'')+'</option>';
          }).join('')+
        '</select></label>' : '')+
...
// numbers, so the panel says so rather than showing the old ones.
async function stCmpSetPrev(idx){
  var o = stPrevOptions[parseInt(idx,10)]; if(!o) return;
  stPrevChoice = { key:o.key };
  var el = document.getElementById('st-cmp-body');
  if(el) el.innerHTML = '<div class="report-no-data">Loading '+stEsc(stPeriodLabel(o.key))+'…</div>';
  await stLoadPrevMonth();
  var head = document.querySelector('#st-cmp-modal .st-cmp-title');
  if(head) head.textContent = stPeriodLabel(stPrevMonth)+' → '+stPeriodLabel(stMonth);
  stRenderCompare();
}
...
// rows (e.g. "as of 31.05.2026") — that date IS the count, so a mid-month sheet
...
// legacy month guesser — kept only so nothing else that may call it breaks
...
function stShowUpload(){
  ...
  box.innerHTML='<div style="background:#fff;border-radius:12px;padding:18px;width:90%;max-width:360px" onclick="event.stopPropagation()">'+
    ...
    '<input class="check-input" id="st-up-month" type="date" style="width:100%;height:38px;margin:4px 0 4px">'+
...
async function stUploadPreview(){
  ...
  try{
    ...
    statusEl.textContent=items.length?(items.length+' items found for '+stPeriodLabel(monthEl.value)+'.'):'No items found — is this the right file?';
...
async function stHandleUpload(){
  ...
  try{
    ...
    if(typeof kToast==='function') kToast('✓ Loaded '+items.length+' items for '+stPeriodLabel(month)+'.');
  }catch(e){
...
async function stApplyUpload(month, items, filename){
  ...
  if(existing.data && existing.data.length){
    ...
    // clearing it first. If this is a SECOND count of the period the right answer
    // is a different date, which keeps both.
    var cRes=await sb.from('stock_take_counts').select('id',{count:'exact',head:true})
      .eq('venue_id',STOCK_VENUE).eq('dept',STOCK_DEPT).eq('month',month);
